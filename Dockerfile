...
RUN composer update
RUN mkdir -p /app/custom-plugins/tinyframework-authentication
RUN composer config repositories.tinyframework-authentication '{"type": "path","url": "custom-plugins/*","options": {"symlink": true}}' --file composer.json
